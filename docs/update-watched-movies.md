...
A CLI tool to update watched movies with additional data from TheTVDB API.

## Usage

```bash
npx update-watched-movies
```

## Description

This tool fetches additional movie metadata from TheTVDB API for all watched movies in the database that don't already have TVDB data. It enriches the movie records with extended information like plot summaries, cast, crew, ratings, and other metadata.

## Behavior

- Queries the database for watched movies (those with a `watchedDate`) that lack TVDB data
...
```bash
# Update all watched movies without TVDB data
npx update-watched-movies
```
